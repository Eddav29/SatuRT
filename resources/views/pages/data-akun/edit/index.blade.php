<x-app-layout>
    ...
    <section class="p-6 lg:px-12 mx-auto max-w-screen-2xl md:p-6 2xl:p-6 flex flex-col gap-y-5">
        ...
        <form action="{{ route('data-akun.update', $id)}}" method="POST" class="p-6 rounded-xl bg-white-snow mt-5 flex flex-col gap-y-7">
            @csrf
            @method('PATCH')
            <div class="space-y-6">
                <x-heading text="Edit Data Akun Penduduk" />
                @include('pages.data-akun.partials.data-akun-form')
            </div>

            <div class="flex justify-start mt-5 space-x-4 px-5">
